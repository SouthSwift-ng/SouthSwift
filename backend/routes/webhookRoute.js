...
router.post('/', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    if (amount !== expectedKobo || (currency && currency !== 'NGN')) {
      // Deterministic mismatch — ACK so Paystack doesn't retry forever; record + log for review.
      ...
      await pool.query(
        ...
        [`Webhook amount mismatch: expected ${expectedKobo}, got ${amount} ${currency}`, deal.id]
      ).catch(() => {});
